...
{
    public class BestTimeToBuyStock : IRunnable
    {
        [Flags]
        enum Operation
        {
        ...
        private void RunWith(int[] prices, bool showDetails = false)
        {
            ...
            if (showDetails)
            {
                foreach (var history in _histories)
                    Logger.LogLine(history);
            }

            var winner = _histories
                .Select(h => new
                {
                    history = h.Substring(0, h.IndexOf('R')),
                    earning = Convert.ToInt32(h.Substring(h.LastIndexOf("R:") + 2))
                })
                .OrderByDescending(h => h.earning)
                .First();

            Logger.LogLine(new string('=', width));
            Logger.LogLine($"Variasions of {prices.Length}: {_histories.Count}");
            Logger.LogLine($"Winner of [{string.Join(',', prices)}] is {winner.history} -> ${winner.earning}");
            Logger.LogLine(new string('=', width));
            Logger.LogLine("");
        }
        ...
        {
            _histories = new List<string>();
            NextOp(prices, 0, Operation.None, 0, 0, "|");

            var winner = _histories
               .Select(h => Convert.ToInt32(h.Substring(h.LastIndexOf("R:") + 2)))
        ...
        private void NextOp(int[] prices, int index, Operation lastOp, int lastPrice, int profit, string history)
        {
            history += CreateHistoryEntry(lastOp, index - 1);

            if (index == prices.Length - 1)
            {
                if((Operation.Buy | Operation.Hold).HasFlag(lastOp))
                {
                    profit += prices[index] - lastPrice;
                    history += CreateHistoryEntry(Operation.Sell, index);
                }

                _histories.Add(history + "R:" + profit);
                return;
            }

            if((Operation.Sell | Operation.None).HasFlag(lastOp))
            {
                NextOp(prices, index + 1, Operation.Buy, prices[index], profit, history);

                NextOp(prices, index + 1, Operation.None, lastPrice, profit, history);
            }

            if((Operation.Buy | Operation.Hold).HasFlag(lastOp))
            {
                NextOp(prices, index + 1, Operation.Hold, lastPrice, profit, history);

                NextOp(prices, index + 1, Operation.Sell, prices[index], profit + (prices[index] - lastPrice), history);
            }
        }

        private string CreateHistoryEntry(Operation operation, int index)
        {
            if ((Operation.Hold | Operation.None).HasFlag(operation) || index < 0)
                return string.Empty;
            
            var opString = operation == Operation.Buy ? "B" : "S";
            return opString + index  + "|";
        }
    }
}
